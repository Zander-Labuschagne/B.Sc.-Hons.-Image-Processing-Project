...
%Calculate the standard deviation of the sum of the vertical pixels
sum = 0;
for vi = 1 : size(vertical_sum, 2)
    sum = sum + (vertical_sum(vi) - avg).^2;
end;
stdev = sqrt(sum ./ (size(vertical_sum, 2) - 1));
fprintf('Vertical Standard Deviation: %f\n', stdev);
fprintf('Vertical Line Factor: %f\n', stdev + vtolerance .* stdev);

   
index = 1;
%Calculate index of left vertical line
for v = round(size(vertical_sum, 2) / 2) : -1 : 1
    %if (vertical_sum(v) > (avg + vtolerance .* avg))
     if (vertical_sum(v) > (stdev + vtolerance .* stdev))
        index = v;
        break;
    end;
end;
...
for v = round(size(vertical_sum, 2) - size(vertical_sum, 2) / 2) : size(vertical_sum, 2)
     %if (vertical_sum(v) > avg + vtolerance .* avg)
      if (vertical_sum(v) > (stdev + vtolerance .* stdev))
         index = v;
         break;
